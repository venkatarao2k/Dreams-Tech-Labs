...
export default function WhatsAppFloat() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      {/* Popup */}
      {showPopup && (
        <div className="glass-strong rounded-2xl p-4 max-w-[220px] shadow-2xl border border-green-500/20 animate-slide-up">
          <button 
            onClick={dismiss}
            className="absolute top-2 right-2 text-slate-500 hover:text-white transition-colors text-sm w-5 h-5 flex items-center justify-center"
          >
            ✕
          </button>
          <p className="text-sm text-slate-200 leading-snug pr-4">
            👋 Need help choosing a project? Chat with us on WhatsApp.
          </p>
          <a 
            href={WA_LINK}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-2 text-xs font-semibold text-green-400 hover:text-green-300 transition-colors block"
